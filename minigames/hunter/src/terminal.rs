...
use common::{network::NetworkMessage, state::{GameState, ServerState, TerminalState}, toolbar::{Docking, ItemState, ToolabarButton, ToolbarItem}};
...
impl Plugin for HunterTerminalPlugin {
    fn build(&self, app: &mut App) {      
        ...
        app.add_systems(OnExit(HunterGameState::On), despawn_basictarget_toolbar_item);
...
fn handle_button_click(
    button_query: Query<(&Interaction, &ToolabarButton), (With<MenuButton>, Changed<Interaction>)>,
    mut client: ResMut<QuinnetClient>,
    terminal_state: Res<State<TerminalState>>,
) {
    for (interaction, toolbar_button) in &button_query {
        if *interaction == Interaction::Pressed && toolbar_button.name == BTN_NAME {
